...
export function computeToolUsage(
	...
	const filter = { ...window, agent: query.agent, project: query.project };
	...
	const allTime = auditLog.aggregateToolUsage({ agent: query.agent, project: query.project });
	const recentSince = new Date(Date.now() - recentWindowDays * 86_400_000).toISOString();
	const recentTools = new Set(
		auditLog
			.aggregateToolUsage({ since: recentSince, agent: query.agent, project: query.project })
			.map((row) => row.tool)
	);
	const everSeen = new Map<string, string>();
	for (const row of allTime) {
		const existing = everSeen.get(row.tool);
		if (existing === undefined || row.lastSeen > existing) {
			everSeen.set(row.tool, row.lastSeen);
		}
	}
	...
	const adapterTools = new Set<string>();
	for (const adapter of adapters) {
		for (const tool of adapter.supportedTools) {
			adapterTools.add(tool.toLowerCase());
		}
	}

	const knownTools = new Set([...adapterTools, ...everSeen.keys()]);
	const ruleRefs = new Map<string, string[]>();
	for (const pattern of Object.keys(manifest.rules)) {
		const tool = ruleToolReference(pattern, knownTools);
		if (tool === undefined) continue;
		const patterns = ruleRefs.get(tool) ?? [];
		patterns.push(pattern);
		ruleRefs.set(tool, patterns);
	}
